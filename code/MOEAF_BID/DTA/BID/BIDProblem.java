...
import ObjEntity.ProductOfPackage;
import ObjEntity.ProductOfContractor;
import ObjEntity.Project;
import ObjEntity.Package;
import ObjEntity.Product;
...
public class BIDProblem extends AbstractProblem {
	...
	//int _numberOfConstrained;
	public BIDProblem(Project projectBID, int numberOfObjectives) {
		super(projectBID.Packages().size() * 2, numberOfObjectives, 1);//, projectBID.Contractors().size() + 1);
		_projectBID = projectBID;
		_numberOfObj = numberOfObjectives;
		//_numberOfConstrained = projectBID.Contractors().size() + 1;
...
//	public int numberOfContrained() {
//		return _numberOfConstrained;
//	}
	...
	public String toString(Solution solution) {
		String result ="";
		for (int i = 0; i < _projectBID.Packages().size(); i++) {
			int indexSelectedJoinContractor = EncodingUtils.getInt(solution.getVariable(i * 2));
			int addedSelectedTime = EncodingUtils.getInt(solution.getVariable(i * 2 + 1));
			Contractor selectedContractor = _projectBID.Packages().get(i).getContractorByIndex(indexSelectedJoinContractor);
			String selectedTime = DateUtil.add(_projectBID.Packages().get(i).from(),addedSelectedTime );
			if(i == 0) {
				
				result += "ProjectProfit: " + (solution.getObjective(0) >0 ? solution.getObjective(0) : -solution.getObjective(0))    + " \r\n";
				result += "ContractorProfit: " + (solution.getObjective(1) >0 ? solution.getObjective(1) : -solution.getObjective(1)) + " \r\n";
				result += "KeepContractorRelationship: "+ (solution.getObjective(2) >0 ? solution.getObjective(2) : -solution.getObjective(2)) + " \r\n";
				result += "ProjectQuality: "+ (solution.getObjective(3) >0 ? solution.getObjective(3) : -solution.getObjective(3))+ " \r\n";
			}
			result += "Package " + i + ": ";
			result += "Contractor: " + selectedContractor.description() + " \r\n";
	...
	public void evaluate(Solution solution) {
		...
		//solution.setConstraint(0, ProjectProfit(x, y) <= 0 ? 0 : ProjectProfit(x, y));
//		
//		solution.setConstraint(1, ContractorProfit(x, y) <= 0 ? 0 : ContractorProfit(x, y));
//		for(int i =0; i < _numberOfConstrained - 1; i++) {
//			solution.setConstraint(i + 1, ContractorProfit(_projectBID.Contractors().get(i), x, y) <= 0 ? 1 :0);
//		}
	...
	public Solution newSolution() {
		// TODO Auto-generated method stub
		Solution solution = new Solution(_projectBID.Packages().size() * 2, _numberOfObj, 1);
		for(int i =0; i < _projectBID.Packages().size(); i++) {
