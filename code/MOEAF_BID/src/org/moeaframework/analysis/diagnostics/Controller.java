		...
		thread = new Thread() {
			...
			public void run() {
				try {
					...
					List<NondominatedPopulation> currentList = executor.runSeeds(numberOfSeeds);
					long longTime = System.currentTimeMillis();
					//System.out.printf("%s : number of seeds: %d, run time: %d, number of list NDP: %d \r\n",algorithmName, numberOfSeeds, (longTime - startTime), currentList.size());
					int idOfNDP = 0;
					for (NondominatedPopulation nondominatedPopulation : currentList) {						
						for (int idSolution = 0; idSolution < nondominatedPopulation.size(); idSolution++) {
							BIDRunner.WriteSolution(nondominatedPopulation.get(idSolution), idSolution,algorithmName, numberOfSeeds,  (longTime - startTime), idOfNDP);
						}
						idOfNDP ++;
					}
